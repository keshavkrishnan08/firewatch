...
def train_surrogate_model(n_train=400, epochs=30) -> dict:
    ...
    vmax = np.nanpercentile(np.where(np.isfinite(true), true, np.nan), 95)
    ax[0].imshow(np.where(np.isfinite(true), true, np.nan), origin="lower", cmap="magma", vmax=vmax)
    ax[0].set_title("physical MTT arrival (min)")
    ax[1].imshow(np.where(np.isfinite(pred), pred, np.nan), origin="lower", cmap="magma", vmax=vmax)
    ax[1].set_title("learned surrogate arrival (min)")
    both = np.isfinite(true) & np.isfinite(pred)
    ax[2].scatter(true[both], pred[both], s=2, alpha=0.3, color="#e4572e")
    lim = [0, vmax]
    ax[2].plot(lim, lim, "--", color="#888")
    ax[2].set_xlabel("MTT (min)")
    ax[2].set_ylabel("surrogate (min)")
    ax[2].set_title(f"val MAE {metrics['val_mae_min']:.0f} min · {metrics['speedup']:.0f}× faster")
    for a in ax[:2]:
        a.axis("off")
    fig.suptitle(f"Learned spread surrogate (FCN, torch/{metrics['device']})")
    fig.tight_layout()
